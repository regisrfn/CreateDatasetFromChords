...
from chord.musical_notes import MusicalNotes
from scipy.signal import find_peaks


class DataFrame:
    def __init__(self,size,folder,chords):
        self.size =  size
        self.folder = folder
        self.chords = chords
    ...
    def create_dataframe(self):
        ...
        for (index, chord_name) in enumerate(names_chords):
            for i in range(size):
                audiofile = f'{folder}/{chord_name}_{i+1}.wav'
                ...
                Y = Y[index_peaks]
                notes = MusicalNotes()
                freqs = notes.get_musical_notes(freqs)
